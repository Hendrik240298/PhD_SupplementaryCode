...
for quantity in ["velocity", "pressure", "supremizer"]:
    S_k = master_ROM.POD["primal"][quantity]["sigs"]
    r = 0
    while (
            np.dot(S_k[0:r], S_k[0:r])
            <= master_ROM.POD["primal"][quantity]["energy"] * master_ROM.POD["primal"][quantity]["TOL_ENERGY"]
        ) and (
            r < np.shape(S_k)[0]
        ):
        r += 1
...
tic = time.time()
master_ROM.solve_primal_velocity()
toc = time.time()
...
plt.plot(
    ...
    master_ROM.drag_force[index_rom_start:index_rom_end], 
    linewidth=3,
    color="#ff7f0e",
    linestyle=":",
    label="ROM"
...
plt.plot(
    master_ROM.fom.time_points[index:index_end],
    master_ROM.lift_force[index_rom_start:index_rom_end],
    linewidth=3,
    color="#ff7f0e",
    linestyle=":",
    label="ROM"
...
plt.savefig(f"plots/drag_lift_parameterized_Re_{Re[parameter_index]}_start_{args.start}_end_{args.end}_step_{args.step}_venergy_{TOTAL_ENERGY['primal']['velocity']}_penergy_{TOTAL_ENERGY['primal']['pressure']}.pdf")
