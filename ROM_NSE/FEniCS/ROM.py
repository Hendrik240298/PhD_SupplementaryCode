...
class ROM:
    ...
    def __init__(
        ...
        self.STARTING_POINT = np.where(self.fom.time_points >= STARTING_TIME)[0][0]
    ...
    def iPOD(self, snapshot, type, quantity, equal_size=False, empty_bunch_matrix=False, FIXED_POD_SIZE=0):
        ...
        if self.POD[type][quantity]["bunch"].shape[1] == self.POD[type][quantity]["bunch_size"] or empty_bunch_matrix:
            ...
            if self.POD[type][quantity]["basis"].shape[1] == 0:
                ...
                if FIXED_POD_SIZE > 0:
                    # choose r as the minimum of the FIXED_POD_SIZE and the number singulare values
                    r_tmp = np.min([FIXED_POD_SIZE, self.POD[type][quantity]["sigs"].shape[0]])
                    r = np.max([r, r_tmp])
            ...
            # update POD with  bunch matrix
                ...
                if FIXED_POD_SIZE > 0:
                    # choose r as the minimum of the FIXED_POD_SIZE and the number singulare values
                    r_tmp = np.min([FIXED_POD_SIZE, S_k.shape[0]])
                    r = np.max([r, r_tmp])
    ...
    def init_POD(self):
        ...
        for i, t in enumerate(time_points[self.STARTING_POINT:]):
            self.iPOD(self.fom.Y["velocity"][:, i+self.STARTING_POINT], type="primal", quantity="velocity", equal_size=False)
            self.iPOD(self.fom.Y["pressure"][:, i+self.STARTING_POINT], type="primal", quantity="pressure", equal_size=False)
        ...
        # for i, t in enumerate(time_points[500:]):
            ...
            self.iPOD(self.fom.Y["supremizer"][:, i+self.STARTING_POINT], type="primal", quantity="supremizer")
    ...
    def solve_primal_velocity(self):
        ...
        self.solution["primal"]["velocity"][:, 0] = self.reduce_vector(
            self.fom.Y["velocity"][:, self.STARTING_POINT], type="primal", quantity="velocity"
        )
        i = 0
        t = 0.
        ...
        self.fom.U_n.vector().set_local(
            np.concatenate(
                (
                    sol_velocity + self.lifting["velocity"],
                    sol_pressure,
                )
            )
        )
    ...
    def solve_primal(self):
        ...
        self.solution["primal"]["velocity"][:, 0] = self.reduce_vector(
            # self.fom.matrix["primal"]["mass"] * 
            self.fom.Y["velocity"][:, self.STARTING_POINT], type="primal", quantity="velocity"
        )
   
        self.solution["primal"]["pressure"][:, 0] = self.reduce_vector(
            self.fom.Y["pressure"][:, self.STARTING_POINT], type="primal", quantity="pressure"
        )
