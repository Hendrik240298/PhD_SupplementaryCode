...
class FOM:
    # constructor
    def __init__(self, t, T, dt, theta, nu):
        self.t = t
        self.T = T
        self.dt = dt
        self.theta = theta
    ...
    def save_solution(self):
        ...
        logging.info(f"Saved as {file_name}")
    ...
    def load_solution_parallel(self):
        ...
        # print(f"{self.SAVE_DIR}solution_{self.mesh_name}_{int(0.1/float(self.nu))}_")
        # pattern = f"solution_{self.mesh_name}_{int(0.1/float(self.nu))}_" + r"_\d{6}\.npz"
        # files = os.listdir(self.SAVE_DIR)
        # files = [
        #     self.SAVE_DIR + f
        ...
        for file in files:
            ...
            if np.array_equal(parameters, tmp["parameters"]):
                ...
                return True
        logging.info(f"No solution found for {int(0.1/float(self.nu))}")
        return False
    ...
    def assemble_lifting_matrices(self, lifting):
        ...
        logging.info("Assembled lifting matrices + rhs")
    ...
    def solve_primal_time_step(self, v_n_vector, p_n_vector):
        ...
        if newton_residuum < self.NEWTON_TOL:
            logging.info(f"Newton residuum: {newton_residuum}")
