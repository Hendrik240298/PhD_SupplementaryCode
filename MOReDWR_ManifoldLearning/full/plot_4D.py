...
if args.yaml_config is None:
    print(
        "No YAML config file was specified. Thus standard config 'result_generation_4D.yaml' is used."
    )
    config_file = "config/result_generation_4D.yaml"
else:
...
#its paramater
print(f"three_largest_errors parameter: {np.array(wanted_parameter)[three_largest_errors[0]][::4]}")
# put theses for values in a string
# string_save = "para" 
# for i in range(4):
#     string_save += f"_{np.array(wanted_parameter)[three_largest_errors[0]][::4][i]}"
# print(string_save)
print(f"three_largest_errors parameter: {np.array(wanted_parameter)[three_largest_errors[1]][::4]}")
print(f"three_largest_errors parameter: {np.array(wanted_parameter)[three_largest_errors[2]][::4]}")
for i in three_largest_errors:
    string_save = "para" 
    for j in range(4):
        string_save += f"_{np.array(wanted_parameter)[i][::4][j]}"
    plt.plot(
        TIME,
...
# write mean of effeciency index to file effeciency_index_mean.txt
