...
if args.yaml_config is None:
    print(
        "No YAML config file was specified. Thus standard config 'result_generation_6D.yaml' is used."
    )
    config_file = "config/result_generation_6D.yaml"
else:
...
with open(SAVE_DIR + "absolute_errors.pkl", "rb") as f:
    absolute_error_estimates = pickle.load(f) # since absolute errors is just sum of errors instead of absolute
...
# ---------------------------------------
"""
NOTE: No usefull ordering available thus just numerating them
"""
...
para_indices = [0,2,4,8,9,13] #i,m,j,k,n,l
...
print(f"three_largest_errors parameter: {np.array(wanted_parameter)[three_largest_errors[0]][para_indices]}")
# put theses for values in a string
# string_save = "para" 
# for i in range(4):
#     string_save += f"_{np.array(wanted_parameter)[three_largest_errors[0]][::4][i]}"
# print(string_save)
print(f"three_largest_errors parameter: {np.array(wanted_parameter)[three_largest_errors[1]][para_indices]}")
print(f"three_largest_errors parameter: {np.array(wanted_parameter)[three_largest_errors[2]][para_indices]}")
...
for i in three_largest_errors:
    string_save = "para" 
    for j in range(6):
        string_save += f"_{np.array(wanted_parameter)[i][para_indices][j]}"
    plt.plot(
        TIME,
    ...
    plt.tick_params(axis="both", which="major", labelsize=13)
    plt.grid()
    plt.savefig(PLOT_DIR + f"cost_functional_{i}_err_{np.array(mean_error_histogram)[i]:.3f}_{string_save}.pdf", bbox_inches="tight")
    plt.close()
    # plt.show()
...
# plt.legend(fontsize=FONT_LABEL_SIZE)
plt.tick_params(axis="both", which="major", labelsize=13)
# plt.xticks([0.0385, 0.03875, 0.039, 0.03925, 0.0395]) 
plt.grid()
...
plt.hist(np.clip(effectivity_index, 0, 25), bins=50)
...
with open(PLOT_DIR + "effectivity_index_mean.txt", "w") as f:
    f.write(f"Mean of effectivity_index: {np.mean(effectivity_index)}")
    f.write(f"Std of effectivity_index: {np.std(effectivity_index)}")
    f.write(f"Max of effectivity_index: {np.max(effectivity_index)}")
    f.write(f"Min of effectivity_index: {np.min(effectivity_index)}")
