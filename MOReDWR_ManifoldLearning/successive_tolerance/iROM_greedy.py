...
class iROMGreedy:
    ...
    def run_ROM(self, POD_given=True):
        ...
        if iteration > self.MEYER_MATTHIES_ITERATIONS and self.MEYER_MATTHIES_EXEC: # LOWER THAN ON FULL_ENRICHMENT SINCE LESSER ITERATIONS PER ENRICHEMNT            #self.MAX_ITERATIONS:
            ...
            for hh in reversed(range(1, len(order))):
                ...
                print(f"max error new: {max_error_new}")
    ...
    def extra_training(self, MAX_ITERATIONS = 1, POD_given=True):
        execution_time = time.time()
        if not POD_given:
            self.init_POD()
        ...
        # 1. Solve primal ROM (since solution does not change outside of loop)
        self.solve_primal()
        ...
        while iteration <= MAX_ITERATIONS:
            ...
            # 4 Enlarge POD basis besides of estimated error value
            print(f"Enrich for largest error @ (i={max_index}: {max_error:.5})")
            self.enrich_ROM_extra_training(max_index)
            fom_solves += 1
    ...
    def enrich_ROM_extra_training(self, i_max):
        # NOTE: only dual is enriched
