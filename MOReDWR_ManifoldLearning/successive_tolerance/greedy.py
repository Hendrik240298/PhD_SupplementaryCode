...
def process_ROM_error_estimation_solely(ROM, EVAL_ERROR_LAST_TIME_STEP):
...
class Greedy:
    ...
    def greedy_enirchment(self, initial_parameter_index=-1):
        ...
        self.iteration = {
            ...
            "TOL": [], # give each iteration the tolerance to identify its outer loop cycle
        }
        ...
        TOL_successive = []
        for i in reversed(range(TOTAL_TOLERANCES)):
            TOL_successive.append(
                self.TOLERANCE * (2**i)
            )
        ...
        new_loop = False # check if new inner loop, since then error estimation can be skipped (exepct in first ouer loop, since no estimation exists)
        
        # -------------------------------------
        # Outer successiv lower tolerance loop
        # -------------------------------------
        for i, TOL_i in enumerate(TOL_successive):
            iter = 0
            # update tolerance for ROMs
            for j, ROM in enumerate(self.ROM_surrogate):
                ROM.REL_ERROR_TOL = TOL_i
            for j, ROM in enumerate(self.ROM_surrogate_estimate):
                ROM.REL_ERROR_TOL = TOL_i
            
            print("################################")
            print(f"# Outer loop TOL_i: {100*TOL_i:2.2f} %")
            print("################################")
            ...
            while iter < self.MAX_ITERATIONS:
                ...
                if not new_loop:
                    ...
                    self.iteration["TOL"].append(TOL_i)
                else:
                    print("Using error estimation from last loop:")
                    print(
                        f"Largest error {relative_error[parameter_index]} at parameter_index {parameter_index}"
                    )                   
                    new_loop = False
                
                if relative_error[parameter_index] < TOL_i:
